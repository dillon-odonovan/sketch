...
class TestConvertOtsToCts(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_slots_length_equals_team_size(self) -> None:
        result = await self._run(llm_spreads=_all_llm_spreads())
        self.assertEqual(len(result.slots), 6)
        self.assertEqual(len(result.team.pokemon), 6)
    ...
    async def test_mixed_bank_and_llm_sources(self) -> None:
        ...
        self.assertEqual(result.team.pokemon[0].evs, bank_spread)
        self.assertEqual(result.slots[0].source.label, "bank")
        self.assertEqual(result.slots[0].source.url, "https://pokepast.es/test")
