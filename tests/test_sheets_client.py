...
def _make_client(
    ...
    return client, svc, request, clock
...
class TestTTLCache:
    async def test_two_rapid_calls_share_one_fetch(self):
        client, _, request, _ = _make_client(ttl=30.0)
    ...
    async def test_call_just_before_ttl_expires_uses_cache(self):
        client, _, request, clock = _make_client(ttl=30.0)
    ...
    async def test_failure_is_not_cached(self):
        ...
        # First execute raises; second execute returns the normal response.
        request.execute.side_effect = [
            RuntimeError("simulated Sheets outage"),
            _SAMPLE_RESPONSE,
        ]
        ...
        assert isinstance(snap, SearchSnapshot)
        # Both the failing and succeeding calls are counted.
        assert request.execute.call_count == 2
...
class TestInvalidate:
    async def test_invalidate_forces_refetch_on_next_get(self):
        ...
        client, _, request, clock = _make_client(ttl=300.0)
        snap1 = await client.get_search_snapshot("MySheet")
        assert request.execute.call_count == 1
    ...
    async def test_invalidate_noop_when_no_entry(self):
        ...
        client, _, request, _ = _make_client(ttl=300.0)
...
class TestDefaults:
    async def test_default_ttl_pulls_from_config(self):
        ...
        client = SheetsClient(MagicMock(), "test-spreadsheet-id")
...
class TestListTabNames:
    ...
    def _make_meta_service(self, response: dict) -> MagicMock:
        svc = MagicMock()
