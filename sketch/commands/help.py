...
def register(tree: app_commands.CommandTree) -> None:
    ...
    async def help_cmd(interaction: discord.Interaction) -> None:
        ...
        msg = (
            ...
            "[format:<reg>] [paste_type:Exact|Recreated|Unspecified] "
            ...
            "page1:<screenshot>`\n\n"
            "`/delete-team [format:<reg>] [url:<paste>] [replica:<code>]`\n"
            "  Remove a team from the database. Provide a Pokepaste URL, a "
            "VRPaste URL, or a Champions Team ID. If both are supplied, "
            "`url` wins. Replica codes match case-insensitively. Removal "
            "is announced in the broadcast channel (if configured) so "
            "others can re-add the team if they disagree.\n"
            "  Examples:\n"
            "    `/delete-team url:https://pokepast.es/abcd1234`\n"
            "    `/delete-team replica:QBXXWXL05U`\n\n"
            "`/edit-team [format:<reg>] [url:<paste>] [replica:<code>] "
            "[description:<text>] [paste_type:Exact|Recreated|Unspecified]`\n"
            "  Fix a team's description and/or paste type without deleting "
            "and re-adding it. Identify the team by Pokepaste URL, VRPaste "
            "URL, or Champions Team ID (if both are supplied, `url` wins). "
            "Provide at least one of `description` / `paste_type` to change. "
            "The Pokepaste URL and Team ID themselves can't be edited — use "
            "`/delete-team` + `/add-team` for that.\n"
            "  Examples:\n"
            "    `/edit-team url:https://pokepast.es/abcd1234 "
            "description:Calyrex-S balance (fixed)`\n"
            "    `/edit-team replica:QBXXWXL05U paste_type:Recreated`\n\n"
            "`/search-teams [format:<reg>] [mon1:<name>] ... [mon6:<name>] "
            ...
            f"Available formats: {formats} (omit `format:` to use the current "
            f"regulation, {config.DEFAULT_FORMAT})\n\n"
