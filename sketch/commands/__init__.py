...
def setup_commands(
    ...
    *,
    ...
    broadcast_channel_id) that don't belong to the SheetsClient.
    `replica_cache` and `anthropic_client` are kwarg-only because they're
    only used by `/add-team` — keyword-only makes the dependency explicit
    at the bot.py call site.
    """
    _register_add_team(
        tree,
        store,
